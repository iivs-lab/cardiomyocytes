...
class NormalizeConfig:
    ...
    `"sequence"` and `"dataset"` read a layer of the range document a measuring run
    left, so they need `range_file` and refuse a `source` of their own. `"given"` is the
    other way round: it carries the span, which is what makes two runs under different
    filters comparable, since a measured range is a property of the filter that shaped
    it.

    Attributes:
        level: Which range a frame is scaled from.
        range_file: The document a measuring run wrote, given `.json` if it has no
            extension. Defaults to `None`, which only `"given"` can do.
        source: The span `"given"` scales from, as `[min, max]`. Defaults to `None`,
            which is what a measured level takes.
        target: The span the output covers, as `[min, max]`. Defaults to `None`, which
            takes the span of the dtype the estimator reads.
    """
...
class Normalization:
    ...
    Settled from the configuration alone, since a measured range is read off a document
    rather than off the dataset: a run can say what it will scale by before it has
    searched for anything to scale, and a mistyped document is refused before the search
    costs anything.

    Attributes:
        described: What went into the scaling, for the settings a later run compares
            against. The ranges themselves and not the document they came from: the same
            path may hold a document that was written again, and a run that scaled by
            other numbers must not read as this one.
        shared: The one scaling that covers every sequence, which is what a given span
            and a dataset range both come to. Defaults to `None`, which is where each
            sequence has its own.
        per_sequence: The scaling for each sequence the document covered, by name.
            Defaults to empty, which is where one scaling covers them all.
    """
    ...
    def normalizers(self, names: Iterable[str]) -> dict[str, FrameNormalizer]:
        ...
        A name with no range of its own is left out rather than refused here. Which
        sequences the run was actually given is settled later, and one the document
        never covered may well be one nobody asked for.
        """
...
def _read_dataset_range(config: NormalizeConfig) -> DatasetRange:
    ...
    Raises:
        ValueError: If no document was named, if the name points at nothing, or if what
            it points at is not a document a measuring run wrote. Each names the setting
            rather than the file, since that is where the fix goes.
    """
...
def build_normalization(config: NormalizeConfig, dtype: torch.dtype) -> Normalization:
    ...
    Args:
        ...
        dtype: The dtype the frames are scaled onto, which is the one the estimator
            downstream reads: `EstimatorConfig.FRAME_DTYPE`.

    Returns:
        The scaling, ready to be asked for the sequences the run turns out to hold.

    Raises:
        ValueError: If the level and the settings beside it disagree, if a measured
            level names no readable document, or if a span is empty.
    """
