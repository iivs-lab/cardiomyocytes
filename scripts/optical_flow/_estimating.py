...
def parse_estimator_config(node: DictConfig | None) -> EstimatorConfig:
    ...
    Unlike the filter there is no estimator that does nothing, so an absent node is a
    run with no way to produce a flow rather than a run that produces them unchanged.
