...
class FlowSourceConfig(PhaseSourceConfig):
    ...
    The same shape whether it reads an acquisition or the cache preprocessing left:
...
class FlowBranchConfig(TreeBranchConfig):
    ...
    Attributes:
        DEFAULT_SUBPATH: Koala's own `<modality>/<precision>/<format>` form for a flow
...
class EvaluationBranchConfig(BranchConfig):
    ...
    Attributes:
        save: Whether to write the document. Defaults to `True`, since it is what the
            stage exists to produce and the flows are the optional half.
...
class FlowInputs(StageInputs["FlowSourceConfig"]):
    ...
    def read(cls, config: DictConfig) -> Self:
        ...
        Raises:
            TypeError: If `filter` describes no kernel, or `estimator` no estimator.
...
def _validate_output(
    ...
    Raises:
        ValueError: If the target writes nothing, or the flows it writes would land on
            the phase they are read from.
    """
...
def build_branches(
    ...
    *,
    ...
    The ranges the frames were scaled by go into the settings, not the path of the
    document they were read from. The same path may hold a document that was written
    again, and a run that scaled by other numbers must not read as this one when a later
    run comes to decide what it may reuse.
    ...
    Raises:
        ValueError: If the target writes nothing, if the flows it writes would land on
            the phase they are read from, or if a policy names something no branch
            offers.
    """
    ...
    *,
    ...
    Args:
        ...
        normalize_config: Where the range the frames are scaled from comes from.
            Defaults to `None`, which takes the dataset range of a document that has to
            be named.
