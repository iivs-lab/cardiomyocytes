...
class FrameDestination:
    ...
    def hook_for(self, source: PhaseFilteredSequence) -> KoalaFrameWriter[Tensor]:
...
class PhaseStageFactory:
    ...
    def stage_for(self, index: int, device: Device) -> SequenceStage[Tensor, Path]:
        ...
        return SequenceStage(sequence).register_hooks(
            *(branch.hook_for(sequence) for branch in self._branches)
        )
...
def build_phase_stages(
    ...
    *,
    ...
    branches: list[SideBranch[PhaseFilteredSequence, Tensor, Path]] = []
    ...
    sequences = build_sequences(source_config, filter_config)

    return PhaseStageFactory(sequences, *branches)
