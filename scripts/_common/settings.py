...
class StageInputs[S: SourceConfig]:
    ...
    everything below holds plain values, checked against the fields they were read for.

    Type Parameters:
        S: The tree this stage reads, which is the one thing about the four that differs
            between stages.
    ...
    def _shared(cls, schema: type[S], config: DictConfig) -> dict[str, Any]:
        ...
        Kept apart from `read` so a subclass composes its own reading out of this one
        rather than restating it: `cls(**cls._shared(...), target=...)`.
