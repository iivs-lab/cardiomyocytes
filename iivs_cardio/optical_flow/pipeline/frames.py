...
class FlowTree(FrameBranch["Named", "Tensor"]):
    ...
    A flow file carries the field alone, so a writer needs nothing off the sequence but
    the name the tree files it under. What a phase writer takes from its source, the
    pixel size and the height scale, is what a flow needs to become a velocity and is
    ...
    def _expected(self, names: Sequence[str]) -> Sequence[str]:
        ...
        A flow is labelled by the frame it starts from, so `N` frames answer `N - 1`
        times and the one left out is at the end. Comparing a record against the
        source's own frames instead would find one name too many every time, and refuse
        to reuse a folder this tree itself wrote.
        """
        return names[:-1]
...
def flow_frame_writer(
    ...
    *,
    ...
    Header-less, so nothing a later run needs travels with the fields: neither the pixel
    size and frame interval that turn a flow into a velocity, nor which frames it came
    from. That is what `record` is for.

    A non finite value is refused rather than written, as it is on the phase side and
    for the same reason: what is written here is what a later run will take as its
    source, and one that got through could not be traced back.
