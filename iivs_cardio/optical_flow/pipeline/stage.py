...
class NormalizedFrameStage(Stage[Tensor, Path]):
    ...
    A stage of its own rather than a step inside the flow stage's `_compute`, because
    two consumers need the same frames: the flow stage reads `i` and `i + 1` to answer
    once, and the evaluation branch reads that same pair back to score the answer
    against it. Normalising inside `_compute` would leave those frames as local
    variables, and a branch normalising again would be a second definition of the same
    thing.

    The window is two for the same reason. The branch reads the pair right after the
    flow stage did, so anything shorter lets go of `i` in between and scales it a second
    time. That is not a wrong answer, since a hook does not fire twice for one index,
    but it is a silent slowdown.

    Args:
        frames: The stage the frames are read from, one per index.
        normalizer: The scaling every frame goes through.
        window: How many recent indices to keep, as for any stage. Defaults to 2, which
            is what a consumer reading a pair needs.
    """
...
class FlowStage(Stage[Tensor, Path]):
    ...
    `flow[i]` runs from frame `i` to frame `i + 1`, so `N` frames answer `N - 1` times
    and the frame with nothing to pair with is the last. That is the labelling this
    project gives every interval quantity, and it is why the estimator is asked for
    `calc` rather than `push`: `_compute` has to be a pure function of the index, and
    what `push` answers depends on what has been pushed into it instead.

    A flow that is not finite everywhere is refused here. Nothing downstream would catch
    it: `grid_sample` reads a non-finite coordinate as out of bounds and the padding
    fills the hole, so every metric comes back with an ordinary number and a broken flow
    reads as a slightly poor reconstruction. Refusing here covers the cache too, and one
    written past this point would be met by a later run with no way back to where it
    came from.

    Args:
        frames: The stage the pairs are read from.
        estimator: The estimator to ask for each pair.
        window: How many recent indices to keep, as for any stage. Defaults to 1, since
            a flow is written and scored where it is computed and nothing reads it back.
    """
...
class FlowSource:
    ...
    Not the phase sequence itself. What a branch needs is the name to file its output
    under, the frames the flows were computed from, and the estimator they came from,
    and the last two are made per run and per device rather than being properties of the
    sequence.

    Attributes:
        name: The name the sequence has in its dataset.
        frames: The frames the flows were computed from, held so that a branch reads
            that same computation rather than making one of its own.
        estimator: The estimator the flows came from, for a branch with another question
            about the same pair. Defaults to `None`, which is what a run reading flows
            from a cache has.
    """
...
class FlowStageRun(StageRun["PhaseFilteredSequence"]):
    ...
    Three stages deep: the sequence reads and filters a phase frame, the frames above it
    are scaled onto what the estimator takes, and the flows above those are what the
    branches watch. The middle one exists so a branch can reach the very frames a flow
    was computed from, which is what makes a score a statement about that flow rather
    than about a second encoding of the pair.

    Args:
        sequences: The sequences to run, in the order they will be offered.
        normalizers: The scaling each sequence's frames go through, by name. One entry
            may be shared by every sequence, which is what a dataset-wide range comes
            to; a per-sequence range gives each its own.
        estimator: The settings the estimator is built from, built once per device
            rather than once per sequence.
        branches: The branches to watch each sequence's flows with. Each is asked for a
            hook with the `FlowSource`, not with the sequence.
        name: The run's name.
    ...
    def _get_estimator(self, device: Device) -> OpticalFlowEstimator:
        ...
        and a run that made one per sequence would pay that for every sequence it has.
        """
