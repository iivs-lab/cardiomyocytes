...
class OpticalFlowEstimator(ABC):
    ...
    Frames are pushed one at a time as `torch.Tensor`s. The tensor carries its own
    device, so CPU and CUDA estimators share a single interface; a CUDA estimator keeps
    its whole round trip on the device (no host transfer), so its output can chain into
    the next GPU stage or be offloaded with `.cpu()`.

    Each `push` returns the flow from the previously pushed frame to the current one, so
    the first call returns `None` (N frames yield N-1 flows). The estimator retains only
    the previous frame, so its memory is O(1) whatever the sequence length: the caller
    consumes or offloads each returned flow rather than accumulating them. `reset`
    starts a new sequence and `calc` is a stateless one-shot for a single pair.

    A subclass pins the concrete dtype and shape of a frame and of a flow, which is the
    one thing this contract leaves open.
...
class EstimatorConfig(ABC):
    ...
    Separate from `OpticalFlowEstimator` so a config, a CLI, or a process-pool recipe
    carries the settings without a live estimator, which cannot cross a process boundary
    while it holds a library object that does not pickle. `build` reconstructs one
    inside the worker, and takes the device because an estimator is bound to one: a cv2
    CUDA algorithm is allocated on whichever device was current when it was made.

    Which devices an algorithm runs on is declared here rather than on the estimator: an
    algorithm with no implementation for one is a fact about the algorithm, not about
    the machinery that streams frames through it.

    What frames the built estimator takes is declared here for the same reason: nothing
    upstream can read the dtype off an estimator that does not exist yet, and a
    normalizer has to scale onto it before one is built.
    ...
    Attributes:
        ...
        FRAME_DTYPE: The dtype the built estimator takes its frames in, which is what a
            normalizer upstream scales onto. Declared without a default, since a wrong
            one would be found in a shape error deep in a library rather than here.
    """
