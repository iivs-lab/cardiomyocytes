...
@pytest.mark.parametrize("kernel", (None, MedianKernel(RADIUS)))
...
def test_source_is_a_plain_conversion_when_no_kernel_is_given(tmp_path):
    # Both views are `DataSequence[Tensor, M]`, which is what lets every read go
    # through one path rather than branching per frame.
    root = _bin_folder(tmp_path / "Bin", _frames())
    sequence = FrameSequence(PhaseBinFolder(root))
    assert not isinstance(sequence.source, FilteredSequence)
    assert isinstance(sequence.source.get_item(0), torch.Tensor)
...
def test_rejects_an_unsupported_device(tmp_path):
    ...
    with pytest.raises(ValueError, match=r"unsupported device 'meta'"):
        FrameSequence(PhaseBinFolder(root), device="meta")
...
def test_from_params_passes_frames_through_when_given_none(tmp_path):
    ...
    frames = _frames()
    root = _bin_folder(tmp_path / "Bin", frames)
    sequence = FrameSequence.from_params(PhaseBinFolder(root))
