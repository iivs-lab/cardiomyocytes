...
def _zero_offset(h: int = 64, w: int = 64) -> torch.Tensor:
    return torch.zeros((2, h, w), dtype=torch.float32)


def _uniform_offset(dx: float, dy: float, h: int = 64, w: int = 64) -> torch.Tensor:
...
def test_backward_warp_with_zero_offset_is_identity():
    img = _textured()
    warped = backward_warp(img, _zero_offset())
...
def test_backward_warp_recovers_known_shift():
    # An offset (dx, dy) = (3, 2) samples `image` at (x + 3, y + 2), so content moves
    # the *other* way and the output equals np.roll(image, (-2, -3)). Check the
    # interior against an independent np.roll.
    img = _textured()
    warped = backward_warp(img, _uniform_offset(3.0, 2.0)).numpy()
...
def test_backward_warp_respects_padding_mode():
    # A large offset pushes the sample point off-grid at the border; `zeros`
    # fills those out-of-bounds samples with 0, unlike `border` replication, so
    # the two disagree on the pulled-in edge.
    img = _textured()
    far = _uniform_offset(40.0, 0.0)  # sample far to the right, past the edge
...
def test_backward_warp_float_image_preserves_dtype_and_fraction():
    ...
    # the mean of its column and the one to its right.
    img = _textured().to(torch.float32)
    out = backward_warp(img, _uniform_offset(0.5, 0.0))
    assert out.dtype == torch.float32
    expected = (img[:, :-1] + img[:, 1:]) / 2  # output columns 0..W-2
...
def test_backward_warp_int16_image_preserves_dtype():
    ...
    out = backward_warp(img, _uniform_offset(3.0, 2.0))
...
def test_backward_warp_batched_matches_individual():
    ...
    offsets = torch.stack([_uniform_offset(3.0, 2.0), _uniform_offset(-2.0, 4.0)])

    batched = backward_warp(images, offsets)
    assert batched.shape == (2, 64, 64)
    assert batched.dtype == torch.uint8
    assert torch.equal(batched[0], backward_warp(img0, offsets[0]))
    assert torch.equal(batched[1], backward_warp(img1, offsets[1]))


def test_backward_warp_rejects_mismatched_batch():
    images = torch.zeros((2, 64, 64), dtype=torch.uint8)
    offsets = torch.zeros((3, 2, 64, 64), dtype=torch.float32)  # batch 3 != 2
    with pytest.raises(Exception, match=r"\[3,2,64,64\]"):
        backward_warp(images, offsets)


def test_backward_warp_rejects_wrong_offset_shape():
...
def test_backward_warp_rejects_non_real_image():
    ...
    with pytest.raises(Exception, match=r"c64\[64,64\]"):
        backward_warp(torch.zeros((64, 64), dtype=torch.complex64), _zero_offset())
    with pytest.raises(Exception, match=r"bool\[64,64\]"):
        backward_warp(torch.zeros((64, 64), dtype=torch.bool), _zero_offset())


def test_backward_warp_rejects_non_float32_offset():
    # offset is pinned to float32 (the internal grid-math dtype); an integer
...
def test_backward_warp_rejects_unknown_padding_mode():
    ...
    with pytest.raises(Exception, match="padding_mode"):
        backward_warp(_textured(), _zero_offset(), padding_mode="wrap")
...
def test_backward_warp_stays_on_cuda():
    ...
    img = _textured().cuda()
    offset = _uniform_offset(3.0, 2.0).cuda()
    assert backward_warp(img, offset).device.type == "cuda"
...
def test_backward_warp_module_matches_function():
    ...
    assert torch.equal(BackwardWarp()(img, offset), backward_warp(img, offset))
    far = _uniform_offset(-40.0, 0.0)
    assert torch.equal(
...
def test_backward_warp_module_batched_matches_function():
    ...
    offsets = torch.stack([_uniform_offset(3.0, 2.0), _uniform_offset(-2.0, 4.0)])
    assert torch.equal(BackwardWarp()(images, offsets), backward_warp(images, offsets))
...
def test_backward_warp_module_rebuilds_grid_on_size_change(monkeypatch):
    ...
    monkeypatch.setattr(torch, "meshgrid", counting)
    module = BackwardWarp()
    module(_textured(), _uniform_offset(1.0, 1.0))  # 64x64
