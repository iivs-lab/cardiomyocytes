...
- **프레임 단위 스트리밍.** 각 단계는 `Iterator → Iterator`이고 **개수를 1:1로
  보존**한다. 그래야 상·하류 훅이 같은 길이의 표를 만들고, 프레임별 기록이
  소스 프레임 하나당 한 행으로 떨어진다.
- **시간 윈도우는 단계가 자기 버퍼로 든다.** accel은 flow 3개(= phase 4개)를
  필요로 하고, 버퍼가 아직 안 찬 동안은 **인덱스는 유지한 채 값만 빈** 항목을 낸다.
  경계 연속성은 청크 overlap이 아니라 이 버퍼가 보장한다.
