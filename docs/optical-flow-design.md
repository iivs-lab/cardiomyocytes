...
(OpenCV 고전 flow)를 현대화하되, **CUDA 백엔드**와 **stateful push API**를 추가한다.
학습형(RAFT류)은 레거시에 없던 새 확장 여지로 남겨 둔다(현재 범위 밖).

- **데이터 시퀀스 읽기·순회는 iivs-lib 0.2.0**에서 온다 (의존성 추가 후 import).
  flow 계산·평가·정규화·3D median 필터는 이 프로젝트가 소유.
...
- 백엔드 매트릭스: **Farneback(cpu/cuda), DualTVL1(cpu/cuda), DeepFlow(cpu만)**.

## 설계 결정 (확정)

1. **백엔드 은닉 + stateful push**: 시퀀스는 `push(frame)` 단일 프레임 스트리밍.
   추정기가 직전 프레임을 보유하고, CUDA는 프레임을 프레임당 1회만 업로드(직전
   프레임 디바이스 상주 → 재업로드 0), 프레임 더블 버퍼 + flow 출력 버퍼 재사용.
2. **백엔드 매트릭스 = 데이터**: `supported_backends` 클래스 속성 + 생성자 검증.
   잘못된 조합은 유효 집합을 명시하는 에러(예: DeepFlow+cuda).
...
class OpticalFlow(ABC):
    ...
    CUDA 백엔드는 프레임당 1회만 업로드하고(직전 프레임 디바이스 상주),
    프레임 더블 버퍼와 flow 출력 버퍼를 재사용한다. 입력은 정규화된 uint8
    단일 채널이어야 한다(정규화는 상류 책임).
    """

    method: ClassVar[str]
    supported_backends: ClassVar[frozenset[Backend]]

    def __init__(self, backend: Backend | str = Backend.CPU) -> None:
        backend = Backend(backend)
        if backend not in self.supported_backends:
            supported = ", ".join(sorted(b.value for b in self.supported_backends))
            msg = (
                f"{self.method!r} does not support the {backend.value!r} backend; "
    ...
    @abstractmethod
    def _build(self):
        """선택된 `self.backend`용 OpenCV 추정기를 생성한다."""

    def reset(self) -> None:
        """보유한 직전 프레임을 비워 새 시퀀스를 시작한다."""
        self._prev = None
        if self.backend is Backend.CUDA:
            self._slot = 0
...
class FarnebackFlow(OpticalFlow):
    method = "farneback"
    supported_backends = frozenset({Backend.CPU, Backend.CUDA})
...
class DeepFlow(OpticalFlow):
    method = "deepflow"
    supported_backends = frozenset({Backend.CPU})  # CPU 전용
...
def create_optical_flow(
    method: str, *, backend: Backend | str = Backend.CPU, params=None
) -> OpticalFlow:
    try:
    ...
    return cls(params, backend=backend) if params is not None else cls(backend=backend)
```

호출자 패턴 (iivs-lib 시퀀스를 직접 순회):

```python
of = create_optical_flow("tvl1", backend="cuda")
...
1. **배치 위치 & 타입체크**: 권장 `iivs_cardio/optical_flow/`(라이브러리, **ty 검사
   대상**). cv2는 타입 스텁이 없어 ty가 untyped로 취급 → `error-on-warning` 하에
   일부 `# ty: ignore`가 필요할 수 있음. 대안: `scripts/`(ty 제외)에 임시로 둠.
...
- **이미 있음**: `opencv-contrib-python`(CUDA build 4.13.0), `numpy>=2`,
  `scikit-image>=0.24`(SSIM/PSNR + scipy 전이).

## 남은 작업 (구현 순서 제안)

1. `iivs_cardio/optical_flow/` 배치 확정 → 위 두 클래스 파일화 (배치 1번 결정 후).
2. **3D median 필터 구현** — 레거시의 타원체(ellipsoidal) footprint median. numba
   대신 `scipy.ndimage.median_filter(footprint=ellipsoid)` 사용 가능(scipy 이미 설치).
   시간축은 순환 버퍼 스트리밍(전체 시퀀스 미적재).
3. **4-mode 정규화** (per-frame / pairwise / sequence / dataset) — flow 전처리, uint8화.
4. **iivs-lib 0.2.0** 의존성 추가 후 시퀀스 로더 배선.
5. 임시 CUDA flow **스크립트** — 위 계산·평가 클래스 + 3D median + 정규화 조립
