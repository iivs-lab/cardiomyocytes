...
- **곁가지 컨테이너의 소유권만 남았다.** `scripts/data/_range.py`의
  `DatasetRangeCollector`는 지금 아무도 쓰지 않는다 — 메인 스트림 이관에서 떼어 냈고,
  자기 테스트만 붙들고 있다.

  아직 정해진 것 없음: 데이터셋 수준 컨테이너가 무엇을 소유하는지, 시퀀스별 결과가 워커에서
  어떻게 돌아오는지, `HookFactory`·`StageFactory`를 프로토콜로 뽑을지. 되살릴 때
  `TargetConfig`의 `save_ranges`·`range_file`과 "둘 다 아니면 거부" 가드가 함께 돌아온다.

  **별도 스크립트로 가르지 않는다.** 메인 스트림은 전처리이고 범위는 그 곁가지일 뿐이라,
  `preprocess_phase` 하나가 둘 다 든다.
...
- **아무것도 로그하지 않는다.** hydra가 잡 디렉터리마다 `preprocess_phase.log`를 여는데 매 실행
...
- **`mpire` insights가 지금 소비자를 잃었다.** `preprocess_sequences`에서 `report_insights` 호출이
...
- **시퀀스 하나가 실행 전체를 죽이면 안 된다.** range 수집기는 유한값이 없는 프레임에서,
  `KoalaFrameWriter`는 목적지가 있고 `overwrite`가 거짓일 때 예외를 던진다. 어느 쪽이든
