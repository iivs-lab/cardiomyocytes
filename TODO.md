...
- **(2)의 스테이지는 `push`가 아니라 `calc`로 배선할 것.** 문제 정의의 규약은 구간량을
  **시작 프레임으로** 라벨링하므로 `flow[i] = calc(phase[i], phase[i+1])`이고, 비는 것은
  **마지막**이지 처음이 아니다. `OpticalFlowEstimator.push`는 `[i-1, i]`의 flow를 `phase[i]`를
  받았을 때 돌려주는 **끝 라벨링**이라, 그대로 스텝에 얹으면 한 칸 밀린다. `calc(prev, curr)`는
  상태가 없고 짝을 그대로 받으므로 `_compute(i)`가 소스에서 `i`와 `i+1`을 당기면 규약이 저절로
  나온다. 당기기 모델이라 선행이 지연이 되지 않는다 — 밀어내기였다면 `flow[i]`를 내려고
  `phase[i+1]`을 기다리며 버퍼링해야 한다.
