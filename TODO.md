...
- **CUDA 할당자가 커널 비용을 잘못 읽게 만든다.** `MedianKernel.apply`가 343 오프셋
  900×900에서 `(S,H,W)` 1.11 GB와 `(S//2+1,H,W)` 557 MB를 동시에 든다. torch의 캐싱 할당자는
  해제된 블록을 세그먼트별로 쥐고 있어서, 가벼운 커널이 먼저 지나가면 총량은 충분한데
  **연속된 큰 블록이 없는** 상태가 된다. 그러면 `release_cached_blocks`가 `cudaFree`로
  장치를 세우고 재시도한다 — 프레임마다.

  진단은 `torch.cuda.memory_stats()["num_alloc_retries"]`이고, 대책은
  `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`(torch 2.12에 있음)다 — 세그먼트가
  제자리에서 자라 새 연속 `cudaMalloc`이 필요 없어진다.
