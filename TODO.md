...
- **컨텍스트 매니저인 훅·곁가지는 한 번만 열린다(AGENTS.md).** 여는 쪽이 같은 객체를 한 번으로
  ...
  거절 메시지는 새로 만들 클래스의 이름을 댄다. `Stage`의 `_walked`는 그대로 두었다: 그쪽은 여는
  것이 아니라 도는 것이라, 한 문장으로 맞추면 메시지가 추상화되면서 구체성을 잃는다.

- **결정 — 값이 정해진 문자열은 `StrEnum`이 아니라 `Literal`로 둔다.** 검토한 것은 `DeviceKind`·
  `PresentPolicy`·`UnsourcedPolicy`·`ShortSequencePolicy`·`RangeLevel`·`KernelShape`·
  `PaddingMode`·`ItemVerdict`다. 대부분 사용자가 YAML이나 CLI에 쓰는 값이고, 그 흐름이 문자열을
  그대로 다룬다: OmegaConf 2.4는 `Literal` 필드를 검증하면서 틀린 값에 사용자가 쓰는 소문자 값을
  나열하지만(`expected one of ['error', 'reuse']`), `StrEnum` 필드에는 멤버 **이름**을 나열한다
  (`expected one of [ERROR, REUSE]`). hydra `instantiate` 경로(`MedianConfig.shape` 등)는 문자열을
  변환하지 않아 `StrEnum` 필드에 평범한 `str`이 들어가고, 맞추려면 frozen dataclass에서 변환 코드가
  필요하다. 호출부와 테스트의 문자열 인자·비교 약 80곳도 멤버로 바꿔야 한다. JSON 기록·피클·`==`
  비교는 어느 쪽이든 같다. 얻는 것(멤버 옆 설명, 멤버 이름 오타의 즉시 실패)이 이 비용보다 작고,
  내부 전용인 `ItemVerdict`만 바꾸면 두 방식이 섞인다.

## 열린 것 — `scripts/_common/compute.py`

- **결정 — 작은 개선들을 어떻게 할지.** 아래와 다른 절에 흩어진 작은 항목을 한 번에 정했다.
  - **`unit`을 진행표시줄 축약과 산문의 명사로 가르고, 워커 수의 복수형을 `quantify`로** — optical
    flow 스크립트를 다시 설계할 때 함께 한다. 항목이 0개인 실행이 `0 worker`라고 쓰는 것도 이때
    닫힌다.
  - **로그 시간 형식 둘** — 두다. `mpire` 통계는 `compute.measure_workers`에서만 나오고, 맞추려면
    `mpire`가 포맷한 문자열을 파싱해야 한다.
  - **워커 로그 레벨의 출처** — 두다. 기본 설정에서는 스테이지 로거와 root의 레벨이 어긋나지 않는다.
    서버 로그를 본 뒤 필요하면 정한다.
  - **워커별 진행표시줄** — 서버 실행에서 멈춘 것처럼 보이는 설정이 실제로 문제가 되는지 본 뒤.
  - **캐시의 `if_unsourced` 기본값** — `keep`으로 둔다. 반쯤 마운트된 원본과 실제로 줄어든 원본이
    똑같이 보이므로 지우는 쪽을 기본으로 둘 수 없다.
  - **이름이 바뀐 시퀀스** — 삭제+추가로 두다. 드물고, 내용으로 알아보는 것은 해시 비용과 오판을
    산다.
  - **필터 캐시를 둘지** — 코드가 아니라 운영 판단이다(§「필터 캐시를 둘지는 조건부다」).
...
**시간 반지름을 바꾸면 이 캐시가 통째로 무효가 된다.** 반지름은 사용자가 직접 정하므로
(§(1)의 「결정 — 시간 반지름은 사용자가 정한다」), 필터 설정이 바뀌면 설정 대조 규칙에 의해
재사용이 전면 금지되고 그 전에 쌓은 캐시는 버린다. **캐시를 채우는 실행은 반지름을 정한 뒤에
할 것.**
...
- **결정 — 시간 반지름은 사용자가 정한다.** 프레임률에서 반지름을 자동으로 정하는 코드는 넣지
  않는다. 반지름은 `filter` 설정으로 직접 주고, `MedianKernel`은 `radius`에 기본값을 두지 않는다.

- **결정 — 전처리의 기본 필터는 `median_ellipsoid_2x2x2`로 둔다.** 연구실의 레퍼런스 필터이고
  10 Hz 데이터에도 써 왔다. 이 항목에 있던 「`(2,2,2)`는 20 Hz 값이고 10 Hz에서 박동 프로파일의
  상대 진폭 ~30%를 잃는다」는 서술은 근거가 없어 지웠다(측정 조건의 기록이 없다). 커널
  docstring의 「반지름은 프레임률을 따라야 한다」는 주장도 같은 이유로 지웠다.
...
- **`data_range`를 `EvaluationDocument`에 넘긴다.** record에서 복원한 `normalize.target`이
  그 값이다. A·B의 `build_branches`가 이것을 넘기지 않고 있어 PSNR/SSIM이 프레임 dtype의
  범위로 채점되는 결함이 있으므로(§「(2) 열린 것」), 같이 고친다.
- **전제 조건 검사**, 프레임 한 장 읽기 전에 시퀀스마다: record의 `sources`가 이번 실행이
  owe하는 `names[:-1]`과 같은가(어긋난 짝), `.npy` 개수가 `len(frames)`와 같은가(반쯤 지워진
  폴더), record의 `filter`가 이번 실행의 필터와 같은가.
...
- **캐시 폴더는 자기가 어디서부터 시작하는지 말하지 않는다.** `FrameWriter`는 도착한
  ...
  **표는 이미 있다.** 각 캐시 폴더의 record가 `sources`로 **인덱스별 원본 프레임 이름**을
  ...
  **`frames`가 아니라 `sources`인 것이 요점이다.** 한 항목이 무엇인지는 호출자가 준
  `source_fn`이 정하므로 파일 이름이 아닐 수도 있고, 흐름 폴더에서 `frames`는 그 폴더가
  가진 flow 파일로 읽히는데 목록에 든 것은 입력이었던 위상 파일이고 개수도 N-1이다.
  §「세는 것이 둘이다」가 평가 문서에서 같은 이름을 거절한 것과 뿌리가 같다.

- ~~**너무 짧아 만들 것이 없는 시퀀스와 만들다 실패한 시퀀스가 같은 예외로 나온다.**~~ **닫혔다**:
  스테이지가 답할 인덱스가 없는 항목은 걷지 않고 `"skipped"`로 세며(`coverage.skipped`, 실패가
  아니다), 어느 항목에도 인덱스가 없으면 워커와 브랜치를 열기 전에 거절한다(§리뷰의 「결정 —
  계산할 것이 없을 만큼 짧은 시퀀스는」). 판단이 스테이지 길이 하나라 힘과 beating profile에도
  그대로 선다. 아래는 결정 전의 기록이다. 힘에 2프레임
...
전부 재현했고, 고친 것은 제목에 취소선을 긋고 어떻게 닫았는지 적었다. 수치는 그때의 측정이다.

- **결정 — optical flow 스크립트는 모드 C를 포함해 전면 재설계한다.** `scripts/optical_flow/`에
  남은 항목(평가의 `data_range`와 채점 관례 표식, 범위 문서의 필터 대조, `FlowTree.required_space`,
  `_normalizing.py`의 빈 문서, 모드 C의 설정과 스크립트, `unit`과 복수형)은 하나씩 고치지 않고 그
  재설계 안에서 다룬다. 전처리 스크립트 쪽의 남은 일은 서버 검증뿐이다.
...
- **Windows에서 문서 커밋이 간헐적으로 거부된다.** `StagedFile.commit`의 `os.replace`가
  ...
  보인다. 같은 폴더에서 `os.replace`만 300회 돌리면 실패가 0회이므로, 파일을 오래 잡는 것이
  아니라 짧게 스치는 잠금이다.

  **재시도는 안전하다.** `os.replace`는 원자적이라 거부된 rename은 아무것도 하지 않은
  상태이고, 다시 시도해도 반쯤 적용될 수 없다. 끝내 안 되면 지금처럼 올린다.

  **자리는 셋 중 하나다.** (가) `kaparoo`의 커밋 — rename 한 곳이 문서·결과·프레임 폴더를 다
  덮는다. (나) 이 저장소의 호출부 — 프레임 폴더는 `FrameWriter`가 `commit()`을 직접 부르므로
  감쌀 수 있고, 문서는 커밋이 `__exit__` 안이라 쓰기 전체를 다시 해야 한다(메모리에 있는 JSON
  이라 비용은 없다). 같은 범위를 덮지만 한 가지 사실이 성격이 다른 세 자리에 적힌다. (다)
  고치지 않고 운영으로 흡수 — 결과 폴더는 남으므로 `reuse`로 다시 돌리면 프레임을 안 읽고
  문서만 다시 쓴다.

  **조건은 `PermissionError`와 Windows로 좁힌다.** POSIX의 EACCES는 진짜 권한 문제라 재시도가
  지연만 만든다. 20 ms × 10회면 계측된 회복 시간(10~17 ms)에 충분하고, 진짜 권한 오류는
  0.2초만 늦게 실패한다. 어디에 넣든 **결함 주입 테스트는 이 저장소에 둔다** — 첫 `os.replace`가
  거부하도록 monkeypatch하고 커밋이 끝까지 가는지 보면, 재시도의 자리가 바뀌어도 계약이 남는다.

- ~~**flow 트리의 `report`가 「wrote 7 frames」라고 쓴다.**~~ `FrameWriter(writes=...)`로 「wrote 7
  flows」.

- **결정 — 계산할 것이 없을 만큼 짧은 시퀀스는 실행을 막지 않고 건너뛴다.** 전에는 `FlowStageRun`이
  2장 미만인 시퀀스가 하나라도 있으면 실행 전체를 거절했다(「holds 1 frames」). 이제 `StageRun.run_stage`가
  그래프를 만든 뒤 **맨 위 스테이지의 `len`이 0이면** 걷지 않고 `"skipped"`를 돌려준다. 몇 장이
  필요한지는 상수가 아니라 스테이지의 길이가 말한다 — flow는 2장, 그 위에 쌓는 가속도는 3장, beating
  profile은 종류에 따라 스스로 정하므로, 어느 러너에서나 규칙이 하나다. 훅을 열지 않으므로 빈 출력이
  남지 않고, 문서는 선택됐지만 결과가 없는 이름을 `coverage.skipped`로 센다. `run_all`은 실패로 치지
  않고(`IncompleteRunError` 없음) 판정 뒤에 「N skipped with no index to compute: ...」를 경고로 남긴다.
  **어느 항목에도** 계산할 인덱스가 없으면 `run_all`이 워커와 브랜치를 열기 전에 `ValueError`로
  거절한다 — 열었다면 아무것도 담지 않은 문서가 커밋되어 고친 실행의 이름을 막는다. 판단은 `StageRun.is_runnable`이
  `build_stage`(훅 없는 그래프, 캐시에 남기지 않는 임시 추정기)의 길이로 하므로 부모 프로세스에서
  장치를 건드리지 않는다. 에러로 끝내도 되는지는 hydra 1.4.0.dev6으로 확인했다: `--multirun`에서 한
  작업이 실패해도 나머지 작업은 끝까지 돌고, 스윕 전체가 exit 1로 끝난다.
  **대가**: 끊긴 취득 같은 데이터 문제로 짧아진 시퀀스가 있어도 실행은 성공으로 끝나므로, 경고 줄과 문서의
  `skipped`를 봐야 드러난다.

- **결정 — 보고 줄에 스테이지나 출력 위치를 붙이는 일은 미룬다.** 한 시퀀스의 보고 줄은 체인 전체의
  훅이 아래에서 위로 한 줄씩 내고, 어느 스테이지·출력의 것인지 말하지 않는다. 지금 경로(전처리,
  optical flow)에서는 줄을 쓰는 훅이 종류마다 하나라 동사(`wrote … flows`, `scored`, `measured`)와
  문서 파일 이름으로 갈린다. 같은 종류의 출력이 한 실행에 둘 이상 생기면(지표마다 프레임 트리 등)
  `wrote 1200 frames`가 여러 줄 나와 모호해진다. 프로젝트가 동작한 뒤 여러 데이터셋에서 실제 로그를
  보고 필요하면 그때 한다. 한다면 설계는 줄이 쓴 곳을 대는 것이다 — `FrameBranch`가 트리 안의
  subpath를 라이터에 넘겨 `wrote 7 flows to Flow/Float/Npy`, `kept 2 sequences already written in
  Flow/Float/Npy`. `Stage`에 이름을 두는 것은 API와 모든 빌더를 바꾸고, 두 스테이지에 걸친 훅의
  소속을 정해야 해서 택하지 않는다.

## 재사용과 기록

- **결정 — 재사용은 소스의 위치를 보지 않는다.** 판정에 쓰는 `settings`에 `source.root`가 없어,
  이름과 프레임 수가 같으면 다른 트리의 결과를 그대로 쓴다(재현: 값을 1.5배 한 트리를 같은
  `run_root`에 `if_present=reuse`로 돌리면 문서가 예전 범위를 「4 reused」로 싣는다). **고치지
  않는다.** 루트는 시퀀스의 내용과 무관한 경로일 뿐이라, 마운트가 바뀌거나 다른 기계에서 돌거나
  청크를 옮겨 병합해도 재사용이 살아 있어야 한다. 이름은 같은데 **내용이 다른 데이터셋**을 같은
  출력에 대는 것은 사용자의 책임이다 — `TreeBranchConfig.if_present`의 docstring이 이미 「이름이
  같게 다시 내보낸 소스는 다시 쓰이지 않고 남으므로, 그런 소스를 따르는 실행은 `overwrite`를
  쓴다」고 말한다.

- **flow 실행이 범위 문서에 기록된 필터를 대조하지 않는다 — 4단계.** 전처리는 거른 프레임의 범위를
  재고 그 필터를 `value_range.json`의 `settings.filter`에 기록하지만, flow 실행(`_normalizing.py`)은
  범위만 읽는다. 취득본을 직접 읽는 flow 실행이 다른 필터를 쓰면 다른 필터로 잰 범위로 정규화하고,
  에러가 나지 않는다(코드로 확인, 실행 미검증). **단순히 두 필터를 비교하면 틀린다**: 캐시를 읽는
  flow 실행은 이미 거른 프레임이라 `identity`가 맞는데, 그 비교는 이를 거절한다. 비교할 것은 프레임이
  실제로 거친 필터 전체 — 소스가 캐시면 그 폴더의 기록(`source.json`의 `settings.filter`)에 flow
  실행의 필터를 더한 것 — 이고, 캐시를 다시 거른 캐시처럼 겹치는 경우의 규칙도 정해야 한다. flow
  스크립트의 일이므로 4단계에서 설계부터 한다.

## 평가의 타당성

- ~~**uint8 프레임을 워프하면 재구성이 반올림된다.**~~ `warp_consistency`가 `frame2`의 float
  사본을 워프한다. 워프 위치는 원래 정확했고(소수 좌표에서 bilinear 보간), 문제는 보간한 값을
  uint8로 되돌리는 반올림이 flow 쪽에만 걸려 기준(`ssim_floor`) 대비 이득을 깎던 것이다.

  고치면서 **반올림이 가리던 것**이 하나 드러났다. float 이미지는 0 오프셋에서도 제자리를 정확히
  읽지 못한다 — 좌표가 `[-1, 1]`로 갔다 오는 float32 왕복 오차가 프레임이 클수록 커진다. 그러면
  같은 두 프레임의 재구성이 mse 0이 아니게 되어 중복 프레임 신호(psnr이 비는 것)가 사라진다.
  `backward_warp`가 오프셋이 정확히 0인 픽셀은 자기 값을 읽게 해서 닫았고, gradient는 샘플링의
  것을 그대로 둔다.

  **남은 절반은 `scripts`다** — `build_branches`가 `data_range`를 넘기지 않는 것, 그리고 채점
  관례가 바뀐 것을 `settings`에 표식으로 남기는 것.
...
- **결정 — 쓸 공간은 실행 전에 브랜치에게 묻고, 모자라면 `OSError(ENOSPC)`로 거절한다.** 확인하는
  코드가 없어서, 수백 GB를 쓰는 실행이 디스크가 찬 시점에야 시퀀스마다 실패했다. 이제 `run_all`이 숏컷
  바로 뒤, 워커와 브랜치를 열기 전에 `StageRun.required_space`로 묻는다. 러너는 계산할 인덱스가 있는
  항목마다 `build_source`(훅을 물을 때와 같은 소스)를 만들어 `SupportsRequiredSpace`를 구현한 브랜치에
  넘기고, 브랜치는 `RequiredSpace(root, adds, replaces)`로 답한다 — 형식을 아는 것이 브랜치이기
  때문이다. `FrameTree`는 위상 `.bin` 헤더 크기 + 픽셀 수 × 4에 기록 파일의 실제 바이트를 더하고,
  다시 쓰는 폴더는 기존 크기를 `replaces`로 준다. 재사용 판단은 `FrameBranch._judge_present`가 디스크를
  바꾸지 않고 하며, 열 때 다시 판단한다. `run_all`은 파일시스템마다 `Σ max(adds − replaces, 0) + 워커
  수 × 가장 큰 replaces`를 여유 공간과 비교한다(교체 중에는 옛 폴더가 새 폴더와 함께 있고, 동시에
  교체되는 것이 워커 수만큼이다). 범위·평가 문서는 수 KB라 묻지 않는다. **`FlowTree`는 아직 답하지
  않는다** — 소스(`Named`)에 크기가 없어 4단계에서 붙인다. 확인 뒤에 줄어드는 공간(같은 디스크의 다른
  작업)은 잡지 못하고, 그때는 시퀀스 실패 → 스테이징 삭제 → 공간 확보 후 `reuse`의 경로를 탄다.

- ~~**훅을 묻기 전에 추정기를 만든다.**~~ `EstimatorConfig.build`의 계약이 「장치에 아무것도 할당하지
  않는다」가 되었다. `OpenCVEstimator`는 설정과 장치만 들고, Backend와 cv2 알고리즘은 첫 flow(또는
  `algorithm`을 읽을 때)에 만든다. 지원하지 않는 장치는 여전히 `build`에서 곧바로 거절한다. 전부
  재사용인 시퀀스, 건너뛴 짧은 시퀀스는 알고리즘을 만들지 않는다. 별도 래퍼 클래스를 두는 안은 위임
  메서드만 늘고 장치 거절이 첫 계산으로 밀려 택하지 않았다. 대가: 워커마다 첫 시퀀스의 `done in`에
  알고리즘 생성(CUDA에서는 컨텍스트 초기화)이 섞인다.

- **CPU와 CUDA의 정규화가 비트 단위로 같지 않다.** 524,288 픽셀 중 4개가 한 단계 다르고(.5
  경계), 그만큼 flow도 갈린다(최대 0.006 px). `FrameNormalizer`를 다시 쓸 때 볼 것.

- **결정 — gaussian 커널의 CPU·CUDA 차이는 받아들인다.** 합성곱 구현이 달라 900²에서 픽셀 약
  20%가 비트 단위로 다르다(최대 1.8e-7). 고치지 않고 테스트로 묶지도 않는다: 필터와 추정기의
  조합은 optical flow까지 끝-끝으로 돌린 결과로 고르므로, 이 크기의 차이는 그 비교 안에서 드러난다.
  지금 `configs/filter`에는 gaussian 옵션이 없다.

## 테스트가 잡지 못하던 것 — 메웠다

코드를 일부러 틀리게 바꿔 테스트가 잡는지 본 27건 중 23건이 그 자리에서 잡혔다. 나머지 넷의
결말은 이렇다.

- **CUDA TV-L1의 `iterations`와 Farneback의 `poly_sigma`** 전달을 빼도 통과했다. 설정이 장치별
  getter로 전부 읽히는지 필드마다 확인하는 테스트를 넣어 둘 다 잡힌다.
- **`flow_magnitude`를 L1 노름으로** 바꾸는 것은 원래 잡히고 있었다. 그 뮤테이션을 돌릴 때
  `test_axes.py`를 빼고 돌린 것이 「통과」로 읽힌 것이다 — 부분 스위트로 판정하지 말 것.
- **CUDA `push`의 버퍼 순서**를 뒤집는 것은 **등가 변형**이다. 어느 슬롯에 새 프레임을 쓰는지만
  바뀌고 교대는 그대로라 답이 같다. 테스트의 공백이 아니다. `push`가 `calc`와 같은 쌍을
  짝짓는지는 CUDA 경로에서도 확인한다.

같은 리뷰에서 비어 있던 자리도 함께 메웠다: 손으로 쓴 bilinear 샘플러와 대조하는 워프 검사(세
패딩 정책), 공식과 대조하는 PSNR, numpy 가우시안 패스와 대조하는 SSIM(11탭·전체 평균 관례를
박는다), 스테이지 그래프를 CUDA에서 도는 검사, 그리고 저장된 flow가 직접 부른 cv2의 답과 비트
단위로 같은지 보는 끝-끝 검사.
