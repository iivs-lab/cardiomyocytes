...
### 문서를 접는 법 — 가중치는 프레임 수가 아니라 채점 횟수다

**`FrameMetrics`는 `ValueRange`와 모양이 다르다.** 후자는 한 양의 두 끝이고, 전자는 이름 붙은
수 여럿이다(`ssim`·`psnr`·`mse`·`mae`에 gain과 FB가 붙는다). 값 타입을 공유하지 않는 결정이
여기서도 맞다.

**가중치는 N이 아니라 N-1이다.** `warp_consistency`는 flow 하나마다 한 번 채점하므로 시퀀스당
채점은 N-1번이다. 위상 프레임 수 N으로 가중하면 시퀀스마다 하나씩 더 센다. §「재사용 판정이
1:1을 전제한다」와 **뿌리가 같다** — (2)가 N을 먹고 N-1을 내는 단이라는 사실이 문서 쪽에서 다시
나타난다. 필드 이름을 `frames`로 두면 그 오해가 코드에 박힌다. **「몇 번 쟀는가」로 부를 것.**

**가중치가 맞으면 청크 병합이 공짜다.** `Σ(nₛ·meanₛ)/Σnₛ = ΣΣxₛᵢ/Σnₛ`이므로 두 단으로 접은
결과가 한 번에 평균낸 것과 **정확히 같고**, 몇 단을 쌓아도 유지된다. 청크별 문서를 다시 접는
것이 같은 연산이다. 가중치를 안 들면 이것이 전부 깨진다 — 600과 1200이 섞여 있어 짧은 쪽
프레임이 두 배로 세어진다.

**`inf` 하나가 문서를 죽인다.** 완벽 재구성이면 PSNR이 `inf`다(실측: `{ssim: 1.0, psnr: inf,
mse: 0.0}`). 프레임 하나가 시퀀스를, 시퀀스 하나가 데이터셋을 `inf`로 만들고, `inf`는 「무한히
좋음」이라 **그 설정이 탐색에서 무조건 이긴다.** 정지 구간에 zero flow가 나오면 일어난다.
정할 것: (가) `inf`를 낸 프레임을 세어 따로 보고하고 평균에서 뺀다, (나) PSNR에 상한을 둔다,
(다) MSE만 접고 PSNR을 접은 MSE에서 유도한다. **(다)가 가장 깔끔하지만** `_metrics`가 못박은
「per sample로 재고 평균낸다, pooled는 다른 양이다」와 정면으로 충돌하므로 그 결정을 다시 여는
선택이다.

**평균에는 증인이 없다.** `CompositeRange`는 `min_index`/`max_index`로 범위를 넓힌 프레임을
추적하게 해주는데 평균은 그러지 못한다. 그리고 **데이터셋 평균이 좋아지면서 시퀀스 하나가
무너지는** 경우가 탐색이 대리 지표를 악용하는 바로 그 모양이다. 지표마다 **최악 시퀀스와 그
이름** 정도는 들 것. 프레임 단위까지는 파트 파일에 이미 있다.
...
- **결정 — (2)의 스테이지는 `calc`로 배선한다. 이유는 성능이 아니라 계약이다.**
