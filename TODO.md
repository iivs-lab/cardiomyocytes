...
- **결정 — 시간 반지름은 사용자가 정한다.** 손상은 창이 덮는 **시간**을 따르지 프레임 수를
  따르지 않으므로, 레거시의 고정 `(2,2,2)`는 20 Hz 값이고 10 Hz 데이터를 과하게 뭉갠다 —
  10 Hz에서 박동 프로파일의 상대 진폭 **~30%**를 잃는 반면 `rz=1`은 잃지 않는다. **결과물이
  곧 박동 프로파일**이므로 이것은 denoising이 아니라 신호 손실이다.

  **그래도 프레임률에서 반지름을 자동으로 정하는 코드는 넣지 않는다.** 데이터셋의 프레임률과
  박동 주기를 알고 반지름을 고르는 것은 실행하는 사람의 몫이고, 반지름은 `filter` 설정으로
  직접 준다. `MedianKernel`이 `radius`에 기본값을 두지 않는 것이 이 결정과 같은 이유다.
