...
## 열린 것 — 단계별 `SourceConfig` / `TargetConfig`의 베이스

(2)와 (3)도 각자의 source·target config를 갖게 된다. 지금은 (1)의 것만 있고
`scripts/data/_process.py`에 산다. 셋이 공유할 것은 `root`와 `subpath`, 그리고 그 둘을
다루는 두 가지다 — **subpath 기본값 채우기**와 **source·target의 (root, subpath) 겹침
검사**. 둘 다 단계마다 반복될 성질이다.

- **지금 뽑지 않는다.** 사례가 하나뿐이라, 뽑으면 (1)의 우연까지 베이스에 박힌다. (2)가
  들어와 두 번째 쌍이 생길 때 **(2)가 모양을 결정하게** 하는 편이 낫다. 그때의 리팩터가
  싼 것은 이미 확인돼 있다: 기본값 채우기는 `frame_folder()` 한 곳, 겹침 검사는
  `_validate_target` 한 곳뿐이다.

- **필드 순서는 이미 맞다.** 두 config 모두 `root`, `subpath`가 앞 두 자리라 dataclass
  상속이 순서를 뒤집지 않는다. omegaconf structured config도 상속 필드를 그대로 읽는다.

- **모양을 가르는 것은 비대칭이다.** source의 기본값은 **클래스 상수**다 — (1)은 Koala의
  `Phase/Float/Bin`. 그런데 target의 기본값은 **소스가 읽은 곳**이라 인자를 받는다
  (`frame_folder(read_at)`). 하나의 베이스가 한 시그니처로 둘을 담을 수 없다. source용과
  target용을 따로 두거나, 공통 베이스에는 필드만 두고 리졸버는 각자 두는 선택이 된다.
  (2)의 flow 트리 레이아웃이 아직 없어 그 클래스 상수가 무엇이 될지도 미정이다.

- **(3)의 target은 프레임 트리가 아닐 수 있다.** 출력이 `{지표: (인덱스, 프레임)}`과
  문서라, `save_frames`도 폴더 통째 교체도 (3)에는 없을 수 있다. 겹침 검사가 붙는 자리는
  「`StagedDirectory`가 시퀀스당 폴더를 통째로 갈아끼우는 곳」이지 target config 일반이
  아니다. (2)·(3)의 출력이 실제로 어느 쪽인지 정해진 뒤에 붙일 것.

- **겹침 검사 자체는 단계 독립이다.** `(source_root, source_subpath)` 대
  `(write_root, target_subpath)` 넷만 있으면 판정된다. 베이스 클래스보다 **함수 하나로 먼저
  올라갈** 후보다 — 클래스 계층을 정하지 않고도 (2)에서 재사용할 수 있다.
