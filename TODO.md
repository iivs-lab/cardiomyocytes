...
- **CUDA 할당자는 크기가 뒤섞일 때만 문제가 된다.** `MedianKernel.apply`가 900×900
  343 오프셋에서 **4.75 GB**를 잡는다(측정). 절반 가까이가 코드에 안 보이는 것이다 —
  `gathered.sort(dim=0).values`에서 `sort`가 값과 **인덱스를 둘 다** 만들고, 인덱스는
  `int64`라 값의 두 배(2.22 GB)인데 `.values`가 곧바로 버린다. `isnan` 마스크도 bool
  한 장(0.28 GB)을 잠깐 든다. 참고로 `topk` 분기는 `k = S//2+1`이라 둘 다 절반인데,
  속도로는 `sort`가 이겨서 343 오프셋은 `sort`를 탄다 — **빠른 분기가 메모리를 더 먹는다.**

  **한 실행에서는 필터 설정이 하나이므로 크기가 매 프레임 같고, 캐싱 할당자가 그것을 잘
  재사용한다.** 시퀀스 양 끝에서 창이 잘려 오프셋이 줄지만(`start = max(0, index - rz)`)
  전부 최대치 이하이고 매 시퀀스 반복된다.

  **60배는 벤치마크에서 나온 것이다.** 커널 설정 11개를 한 프로세스에서 연달아 돌렸더니
  같은 코드가 `cuboid (3,3,3)`에서 **8.46초 대 139 ms**로 읽혔다. 크기가 제각각인 블록이
  세그먼트에 흩어져 다음 큰 요청에 연속 공간이 없으면 `release_cached_blocks`가 `cudaFree`로
  장치를 세우고 재시도한다 — 프레임마다. 생산 경로가 아니다.

  **진짜 위험은 온라인 모드다.** (1)(2)(3)이 한 프로세스에서 이어 돌면 필터의 4.75 GB,
  추정기 작업공간, flow 텐서, 지표 중간값이 같은 풀에 섞인다. 게다가 11 GB 카드에서
  4.75 GB를 쓰면 **OpenCV의 별개 할당자**에게 남는 여유가 적다 — `cv2.cuda`는 torch의
  캐시를 모르므로 torch가 쥔 메모리를 추정기가 못 얻을 수 있고, `tensor_to_gpumat`이
  프레임 복사는 없애도 추정기 내부 작업공간은 OpenCV 쪽 할당이다.
...
- **파이프라인·컨테이너는 처음부터 다시 설계 중.** 트리에 있는 `Slot`·`Node`·`Steps`·
  `FieldWriter`·range collector들은 수렴하지 못한 대화에서 나온 것이다. 코드는 남아 있고
  통과하지만, **확장할 것이 아니라 새로 판단할 대상**이다.
